...
def get_uuid_to_name_map() -> dict[str, str]:
    try:
...
def get_player_list() -> tuple[dict[str, list[Player] | str], int]:
    try:
...
def get_player_stats(player_uuid: str) -> tuple[dict[str, Any] | str, int]:
    try:
        try_paths = [
            player_uuid,
